...
var userdetails, orderdetails, messagetext, itemcount, total, datemessagel
var closedmessage, closingtime, queriesmessage;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // build last order time for current day   
    const lastorder = new Date();
    lastorder.setFullYear(year, month, day);
    lastorder.setHours(closingtime, '00', '00', '00');
    // validate if orders are freezed for today
    if (Date.parse(curtime) < Date.parse(lastorder)) {
        ...
        deltable = document.getElementById("delslot");
        deltable.style.display="none";

        bannertable = document.getElementById("banner");
        bannertable.style.display="none";

        veglisttable = document.getElementById("veglist");
        veglisttable.style.display = "none";

        orderbutton = document.getElementById("order");
        orderbutton.style.display = "none";

        const orderclosed = document.getElementById("orderclosed");
        orderclosed.innerHTML = closedmessage;  
        
        const queries = document.getElementById("queries");
        queries.innerHTML = queriesmessage;  
    }
});
